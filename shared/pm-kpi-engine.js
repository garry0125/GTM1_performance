(function () {
  ...
  function aggregateGoogleKpi(rows, columns) {
    const cols = getGoogleColumns({ columns });
    let impressions = 0;
    let clicks = 0;
    let cost = 0;
    rows.forEach((row) => {
  ...
  function refreshPmKpiSummary() {
    ...
    if (state.google?.rows?.length) {
      ...
      const metrics = aggregateGoogleKpi(filtered, state.google.columns);
  ...
  function buildPmDailySeries(start, end) {
    ...
    let googleRows = null;
    let googleDateCol = null;
    let googleColumns = null;
    if (state.google?.rows?.length) {
      googleColumns = state.google.columns;
      googleRows = prepareGoogleRows(state.google.rows, googleColumns);
      googleDateCol = getGoogleColumns(state.google).DATE;
    }
    ...
    return dates.map((d) => {
      ...
      if (googleRows) {
        platformRaws.google = aggregateGoogleKpi(filterRowsByDate(googleRows, googleDateCol, d, d), googleColumns);
      }
