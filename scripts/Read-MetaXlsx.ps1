function Get-LatestMetaFile {
    param([string]$MetaDir)
    $files = Get-ChildItem -Path $MetaDir -Filter "*.xlsx" -File -ErrorAction SilentlyContinue
...
function Export-MetaDataJson {
    ...
    $latest = Get-LatestMetaFile -MetaDir (Join-Path $RootDir "Meta")
    if (-not $latest) {
        throw "Meta 폴더에 YYYYMMDD 형식의 xlsx 파일이 없습니다."
    }

    $parsed = Read-MetaXlsx -Path $latest.Path
