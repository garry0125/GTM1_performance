...
function Get-LatestGoogleFile {
    param([string]$GoogleDir)

    if (-not (Test-Path -LiteralPath $GoogleDir)) { return $null }

    $files = Get-ChildItem -Path $GoogleDir -Filter "*.csv" -File -ErrorAction SilentlyContinue
...
function Export-GoogleDataJson {
    ...
    $latest = Get-LatestGoogleFile -GoogleDir (Join-Path $RootDir "Google")
    if (-not $latest) {
        throw "Google 폴더에 YYYYMMDD 형식의 csv 파일이 없습니다."
    }

    $parsed = Read-GoogleCsvRows -Path $latest.Path
