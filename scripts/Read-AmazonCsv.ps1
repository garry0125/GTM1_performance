...
function Get-LatestAmazonFile {
    param([string]$Dir)

    if (-not (Test-Path -LiteralPath $Dir)) { return $null }

    $files = @(
        Get-ChildItem -Path $Dir -Filter "*.csv" -File -ErrorAction SilentlyContinue
        Get-ChildItem -Path $Dir -Filter "*.xlsx" -File -ErrorAction SilentlyContinue
    ) | Where-Object { $_ }
    ...
    foreach ($file in $files) {
        ...
        if ($base.Length -lt 8) { continue }
        $dateStr = $base.Substring($base.Length - 8)
        if ($dateStr -notmatch '^\d{8}$') { continue }
        if (-not $best -or $dateStr -gt $best.Date) {
            $best = [PSCustomObject]@{
                Name = $file.Name
                Date = $dateStr
                Path = $file.FullName
...
function Export-AmazonDataJson {
    ...
    $amazonDir = Join-Path $RootDir "Amazon"

    $spLatest = Get-LatestAmazonFile -Dir (Join-Path $amazonDir "SP")
    $sbLatest = Get-LatestAmazonFile -Dir (Join-Path $amazonDir "SB")

    if (-not $spLatest -and -not $sbLatest) {
        throw "Amazon\SP, Amazon\SB 폴더에 YYYYMMDD 형식의 파일이 없습니다."
    }
    ...
    if ($spLatest) {
        try {
            $spColumns = @(Get-CsvHeaders -FirstRow (Import-Csv -LiteralPath $spLatest.Path -Encoding UTF8 | Select-Object -First 1))
            $spRows = @(Read-AmazonDataRows -Path $spLatest.Path -AdType 'SP')
        }
        catch { Write-Warning "SP read failed: $($_.Exception.Message)" }
    }
    if ($sbLatest) {
        try { $sbRows = @(Read-AmazonDataRows -Path $sbLatest.Path -AdType 'SB' -SpColumns $spColumns) }
    ...
    $sourceDate = ''
    if ($spLatest -and $spLatest.Date -and $sbLatest -and $sbLatest.Date) {
        $sourceDate = [Math]::Max([int]$spLatest.Date, [int]$sbLatest.Date).ToString()
    }
    elseif ($spLatest -and $spLatest.Date) { $sourceDate = $spLatest.Date }
    elseif ($sbLatest -and $sbLatest.Date) { $sourceDate = $sbLatest.Date }

    $payload = [ordered]@{
        sourceFiles = [ordered]@{
            SP = if ($spLatest) { $spLatest.Name } else { $null }
            SB = if ($sbLatest) { $sbLatest.Name } else { $null }
        }
        sourceDate = $sourceDate
        updatedAt  = (Get-Date).ToString("yyyy-MM-ddTHH:mm:ss")
        spColumns  = $spColumns
        sbColumns  = if ($sbLatest) {
            $sbParsed = Read-MetaXlsx -Path $sbLatest.Path
